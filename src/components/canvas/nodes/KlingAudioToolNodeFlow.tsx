/**
 * KlingAudioToolNodeFlow - Kling 平台音频工具节点
 * 用于承载可灵平台音频能力（文生音效/视频生音效/TTS/自定义音色等）
 */
import React, { memo, useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { Handle, Position, NodeProps } from '@xyflow/react'
import { Copy, Trash2, Wand2 } from 'lucide-react'
import { useGraphStore } from '@/graph/store'
import { KLING_AUDIO_TOOLS } from '@/config/models'
import { runKlingToolNode } from '@/lib/workflow/klingTool'

interface KlingAudioToolNodeData {
  label?: string
  toolKey?: string
  payload?: string
}

const TOOL_OPTIONS = (KLING_AUDIO_TOOLS as any[]).map((t: any) => ({ key: t.key, label: t.label }))

const getToolConfig = (toolKey: string) => {
  const k = String(toolKey || '').trim()
  return (KLING_AUDIO_TOOLS as any[]).find((t: any) => t.key === k) || (KLING_AUDIO_TOOLS as any[])[0]
}

export const KlingAudioToolNodeComponent = memo(function KlingAudioToolNode({ id, data }: NodeProps) {
  const nodeData = data as KlingAudioToolNodeData
  ...
  const getDefaultPayload = useCallback((k: string) => {
    switch (String(k || '').trim()) {
      case 'kling-text-to-audio':
        return { prompt: '', duration: 5 }
      case 'kling-video-to-audio':
        return { video_id: '', video_url: '', sound_effect_prompt: '', bgm_prompt: '', asmr_mode: false }
      case 'kling-tts':
        return { text: '', voice_id: 'genshin_vindi2', voice_language: 'zh', voice_speed: 1.0 }
      case 'kling-custom-voices':
        return { voice_name: '', voice_url: '', video_id: '', callback_url: '', external_task_id: '' }
      default:
        return {}
    }
  }, [])
  ...
  const handleDuplicate = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (!node) return
      store.addNode('klingAudioTool', { x: node.x + 50, y: node.y + 50 }, { ...node.data })
    },
    [id]
  )
    ...
    <div className="relative" onMouseEnter={() => setShowActions(true)} onMouseLeave={() => setShowActions(false)}>
      ...
      >
        <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--border-color)]">
          <span className="text-sm font-medium text-[var(--text-secondary)]">{nodeData?.label || 'Kling 音频工具'}</span>
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--text-secondary)] py-2 border-t border-[var(--border-color)]">
            ...
            <span className={`px-2 py-0.5 rounded-full ${inputStatus.audios > 0 ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800'}`}>
...
export default KlingAudioToolNodeComponent
