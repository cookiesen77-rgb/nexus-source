            ...
            {projects.length === 0 ? (
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {projects.map((p) => (
                  <div
                    key={p.id}
                    className="group relative h-[220px] overflow-hidden rounded-[20px] border border-[var(--border-color)] bg-[var(--bg-secondary)] p-4"
                  >
                    <button
                    ...
                    <div className="mt-1 text-xs text-[var(--text-secondary)]">{new Date(p.updatedAt || p.createdAt).toLocaleString()}</div>

                    <div className="pointer-events-none absolute right-3 top-3 flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <div className="pointer-events-auto flex gap-1">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if (!window.confirm(`确定删除「${p.name}」？此操作不可恢复。`)) return
